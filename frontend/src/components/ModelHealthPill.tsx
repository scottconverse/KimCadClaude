...
export default function ModelHealthPill() {
  ...
  const problem =
    model !== null && model.backend !== 'cloud' && !(model.running && model.model_present)
      ? !model.running
        ? // UX-COLD-001: TinkerQuarry manages its own AI engine now — guide to the in-app setup, never
          // "go start Ollama yourself" (the old dead-end).
          'Your local AI isn’t ready yet — finish setup (the wizard’s “Set up TinkerQuarry’s AI”) to design.'
