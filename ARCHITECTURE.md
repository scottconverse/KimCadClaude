...
| `moonraker_connector.py` | A Moonraker (Klipper) REST connector over stdlib `urllib` — covers Creality-Klipper / Voron / RatRig / Mainsail / Fluidd. Optional `X-Api-Key` (Moonraker often runs unauthenticated on a LAN). Maps Klipper `print_stats.state` onto the normalized states; an unrecognized state reports `error`, never a false "ready." Now also implements `pause` / `resume` / `cancel` (the `PrinterConnector` Protocol declares them). |
| `snapmaker_connector.py` | `SnapmakerConnector` (Klipper/Moonraker, multi-toolhead) — **foundation for the in-development multi-material support; the Snapmaker U1 printer is pulled from the catalog until multi-material ships** (see `ROADMAP.md`). `SnapmakerConnector(MoonrakerConnector)` overrides `capabilities()` to detect the active extruder objects and report `toolhead_count`, and `status()` to build `toolhead_temps` (only the extruders reporting a numeric temperature) plus `nozzle_temp_c`; all send/job/pause/resume/cancel is **inherited** from `MoonrakerConnector`. The connector code stays in the tree as **dormant, gated** groundwork — every shipped printer is currently single-material because the geometry path emits a single solid mesh, so there is no per-region assignment to drive a second toolhead. The multi-toolhead exemplar in the codebase. |
