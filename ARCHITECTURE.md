...
| `snapmaker_connector.py` | `SnapmakerConnector(MoonrakerConnector)` for the **Snapmaker U1** (Klipper/Moonraker-based, **4-toolhead**). Overrides `capabilities()` to detect the active extruder objects and report `toolhead_count`, and `status()` to build `toolhead_temps` (T0–T3, only the extruders reporting a numeric temperature) plus `nozzle_temp_c`. All send/job/pause/resume/cancel is **inherited** from `MoonrakerConnector`. The multi-toolhead exemplar in the codebase. |
