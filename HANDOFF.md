# KimCad — Handoff (2026-06-10 — STAGE 11 DONE: THE BETA IS BUILT. KimCad-Setup-0.9.0b1.exe proven on the real installed tree; tagged `stage-11` + `beta`. Stages 0–11 tagged. Next: Kim runs the beta on her printers — docs/beta/first-hardware-contact.md.)
...
- **Active task:** **ALL STAGES COMPLETE — the beta is built and gate-passed.** What remains is PHYSICAL: Kim runs `KimCad-Setup-0.9.0b1.exe` (the GitHub `beta` release; SHA-256 beside it) on her box and her printers per `docs/beta/first-hardware-contact.md`; whatever only shows on metal gets fixed then. Post-beta watch items: the Stage-11 gate deep-dives (docs/audits/stage-11/beta-gate-2026-06-10/) + dispositions. *(Historical: Stage 10 was DONE (tagged `stage-10`: SPA direct print via SendPanel with confirm-is-the-POST; Bambu-native LAN connector mock-tested with fail-closed hardware edges; in-app model downloads with progress; gate package at `docs/audits/stage-10/audit-team-2026-06-10/`). Next = Stage 11 (Windows installer + beta gate, FINAL)** — WebView2 shell, bundle Python+SPA+OpenSCAD+OrcaSlicer(+PrintProof3D), Ollama detect/install + both model pulls (reuse `/api/model-pull`), version normalization, clean-profile install test, beta gate audit-team at 0/0/0/0/0 on the installed artifact, tag `beta`. Note also the Stage-10 watchlist: a minimal in-app connections card belongs in Stage 11; first-hardware-contact checklist before the first real print at Kim's. Each: per-slice audit-lite → stage gate (audit-team [+ wiring-audit for UI]) → 0/0/0/0/0 → merge → tag. Tracker: `docs/audits/RUN-LEDGER-2026-06-05.md` (Phase C table). **One open item for Scott:** ENG-006 — physically VERIFY the Bambu P2S/A1 build-volume envelopes (`config/default.yaml` VERIFY markers); mitigated by the Stage-5 sliceable-footprint cap.
...
- **🔧 STAGE 8.5 (Usability) — `(HISTORICAL: this section was written mid-build; Stage 8.5 is now DONE, merged + tagged `stage-8.5` — see the RESUME box up top.)`** Original branch `stage-8.5-usability` (off `main`/`stage-7`).
  **= make the working core loop actually usable; 8.5-first, BEFORE the Stage 8 CadQuery backend**
  ...
  here) *(resolved at the stage-10 tag: it did NOT ship in Stage 10 — recorded as
  deferred-not-dropped in the CHANGELOG Stage 10 entry)*. audit-lite (independent) → 1 Major + 1 Minor + 1 Nit (stale API-contract doc; zero-volume
